...
%% Adapted by M. Lanjouw to include an additional seaweed input
%% Use IC_W2_seaweed, IC_H9_seaweed or IC_SM7_seaweed
%% Uses: CO2SY S
...
D_bio_0=1e-6*2.32*((Foc+Focs)*1e2)^0.85;      %[m2/a] surf bioturb coeff, Archer et al (2002)
lambda_b = 0.08;      %[m] characteristic bioturbation depth
% Two variables for the bioturbation coefficient are created to allow one
% changing flux over time. The s stands for added seaweed and is used for
% the time period in which seaweed is added. The nat stands for natural
% background conditions and is used for the times when no additional
% seaweed is added.

D_bio_s=D_bio_0*exp(-(depths./lambda_b).^2).*((dO2w/1e-3)/((dO2w/1e-3)+20)); %[m2/a] bioturb coeff during seaweed addition
D_bio_0=1e-6*2.32*((Foc)*1e2)^0.85; 
D_bio_nat= D_bio_0*exp(-(depths./lambda_b).^2).*((dO2w/1e-3)/((dO2w/1e-3)+20)); %bioturbation with just background organic carbon flux


%% irrigation (for solutes)
alpha_0=11*(atan(((5*(Foc+Focs)*1e2-400))/400)/pi+0.5)-0.9...
    +20*((dO2w/1e-3)/((dO2w/1e-3)+10))*exp(-(dO2w/1e-3)/10)*(Foc+Focs)*1e2/((Foc+Focs)*1e2+30);    %[/a] from Archer et al (2002)
lambda_i=0.05;    %[m] characteristic irrigation depth
%Similarly to bioturbation coefficient, two irrigation are created

alpha_s=alpha_0.*exp(-(depths/lambda_i).^2);    %[/a] irrigation during seaweed addition
alpha_0=11*(atan(((5*(Foc)*1e2-400))/400)/pi+0.5)-0.9...
    +20*((dO2w/1e-3)/((dO2w/1e-3)+10))*exp(-(dO2w/1e-3)/10)*(Foc)*1e2/((Foc)*1e2+30);
alpha_nat=alpha_0.*exp(-(depths/lambda_i).^2); 
...
delta_D_bio_s = -2*depths.*D_bio_s/lambda_b^2; % [not used in Julia] during seaweed addition
delta_D_bio_nat = -2*depths.*D_bio_nat/lambda_b^2; % [not used in Julia] before and after seaweed addition

% biodiffusion depth-attenuation: see Boudreau (1996); Fiadeiro and Veronis (1977)
Peh=w_nat.*z_res./(2*D_bio_nat);      %one half the cell Peclet number (Eq. 97 in Boudreau 1996)
% when Peh<<1, biodiffusion dominates, when Peh>>1, advection dominates
sigma_nat=1./tanh(Peh)-1./(Peh);  %Eq. 96 in Boudreau 1996
Peh=w_s.*z_res./(2*D_bio_s);
sigma_s=1./tanh(Peh)-1./(Peh); 
...
if rerun == 0 %concentrations set to zero for solids, bottom water values for not-too-sensitive solutes
    ...
    pfoc(1,1:ndepths)=0; 
    procs(1,1:ndepths)=0; %for seaweed refractory fraction
    psocs(1,1:ndepths)=0; %for seaweed slow degrading fraction           
    pfocs(1,1:ndepths)=0; %for seaweed fast degrading fraction
...
elseif rerun==1 %if it is a rerun, initial conditions are concentrations from last time step
    ...
    psoc=psocic';
    pfoc=pfocic'; 
    procs(1,1:ndepths)=0'; % no initial conditions for seaweed POC
    psocs(1,1:ndepths)=0';
    pfocs(1,1:ndepths)=0';
    pFeOH3=pFeOH3ic';
    pMnO2=pMnO2ic';
...
% certain variables are commented out because the chance based on whether
% seaweed is added and are therefore added to the loop
%alpha_j = alpha(j); 
z_res_j = z_res(j);
z_res2_j = z_res2(j);
DFF_j = DFF(j);
...
for i=i:t_length-1
  ...
  %F_TAi=(D_dtalk*phi(1)*(dtalkf(1,:)-dtalkw)./dbl)';
   ...
   % Aragonite dissolution rate from Dong et al. (2019) EPSL
    ...
    %For seaweed, time specified so that the reaction rate are not altered
    %when the additional seaweed flux is absent
    if not(Focs == 0) && t_start<=i
        ...
        TotR_psocs(1:ndepths) = 0;
        TotR_pfocs(1:ndepths) = 0;
    end


    %% Start and end seaweed organic carbon flux
    if  t_start>=i || t_end<=i % Change fluxes to zero and use all coefficients for natural background conditions
        Frocs=0; 
        Fsocs=0; 
        Ffocs=0; 
        D_bio=D_bio_nat;
        ...
        u_j = u_nat(j);
        w = w_nat;
    else % use flux specified for species and coefficients based of both fluxes
        Frocs=Froc_s; 
        Fsocs=Fsoc_s; 
        Ffocs=Ffoc_s; 
        D_bio=D_bio_s;
